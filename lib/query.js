...
function dbQuery(db, query) {
    ...
    if (queryKeys.length === 0) return Object.keys(db);
    ...
    for (const qName of Object.keys(query)) {
        ...
        switch (qName) {
            ...
            case "$or":
                ...
                if (!qVal.every(qry => validateQuery(qry))) return false;
                break;

            case "$not":
                if (!validateQuery(qVal)) return false;
                break;
...
function evalQuery(doc, query) {
    for (const qName of Object.keys(query)) {
        ...
        switch (qName) {
            ...
            case "$not": {
                if (evalQuery(doc, qVal)) return false;
                break;
            }
            case "$where": {
                if (!qVal(doc)) return false;
                break;
            }
            default: {
                if (typeof qVal === "object") {
