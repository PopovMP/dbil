...
function dbAction(req, res, env, methodName, params)
{
	...
	for (const param of Object.keys(params) ) {
		...
		try {
			...
			const argObj  = JSON.parse(argText)

			if (typeof argObj !== 'object' || Array.isArray(argObj) || argObj === null)
				return reportError(res, 'Parameter is not an object: ' + param)

			methodArgs.push(argObj)
		}
		catch(e) {
	...
	const data = db[methodName].apply(null, methodArgs)
