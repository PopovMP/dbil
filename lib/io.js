 ...
 * @param { string } filePath
 * @param { Object } db
 * @param { (err: Error|string|null) => void } callback
 *
 * @return { void }
 */
function saveDb(filePath, db, callback) {
    ...
    safeWriteFile(filePath, content, {encoding : "utf8"}, callback);
}
 ...
 * @property { (err: Error|string|null) => void } callback
 */
 ...
 * @return { void }
 */
function safeWriteFile(filePath, content, options, callback) {
    ...
    if (busy[filePath]) {
        ...
        /** @type { (err: Error|string|null) => void } */
        ...
        queue[filePath] = {timeoutID, filePath, content, options, callback};
        return;
    }
    ...
    writeFile(filePath, content, options, (err) => {
    ...
    function repeatWriteFile(filePath) {
        ...
        safeWriteFile(job.filePath, job.content, job.options, job.callback);
    }
}
